...
open class PTCardTabBar: UIView {
    ...
    public lazy var indicatorView: PTIndicatorView = {
        ...
        DispatchQueue.main.async { [weak self] in
            self?.stackView.arrangedSubviews.forEach {
                if let button = $0 as? UIControl {
                    button.removeTarget(self, action: #selector(PTCardTabBar.buttonTapped(sender:)), for: .touchUpInside)
                }
            }
        }
    }
    ...
    public func add(item: UITabBarItem) {
        self.items.append(item)
        self.addButton(with: item.image!)
    }

    public func remove(item: UITabBarItem) {
    ...
    public func updateIcon(at index: Int, image: UIImage) {
        let allButtons = buttons()
        guard index < allButtons.count else { return }
        allButtons[index].setImage(image, for: .normal)
    }

    @objc public func buttonTapped(sender: PTBarButton) {
